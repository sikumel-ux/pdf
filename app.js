...
async function fetchFiles() {
    ...
    list.innerHTML = `<div style="text-align:center;padding:30px;"><i class="fa-solid fa-circle-notch fa-spin fa-2x" style="color:var(--deep-blue)"></i></div>`;
    try {
...
function renderFiles(files) {
    ...
    list.innerHTML = files.map(f => {
        ...
        return `
        <div class="file-row">
            <i class="fa-solid ${icon}" style="color:var(--deep-blue); font-size:20px;"></i>
            <div class="file-info">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(uploadBtn) uploadBtn.onclick = async () => {
        const file = fileInput.files[0]; if(!file) return showToast("⚠️ Pilih file dulu!");
...
async function deleteFile(n, s) { if(confirm(`Hapus file ${n}?`)) { await fetch(API_URL, { method: "POST", body: JSON.stringify({ action: "delete", name: n, sha: s }) }); fetchFiles(); } }
